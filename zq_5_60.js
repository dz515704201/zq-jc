...
const $ = new Env('中青看点5分钟60分钟任务领取');
let zqerciboxbody= $.isNode() ? (process.env.zqerciboxbody ? process.env.zqerciboxbody : "") : ($.getdata('zqerciboxbody') ? $.getdata('zqerciboxbody') : "")
let zqboxbodyArr = []
let zqboxbodys = ""
if (zqerciboxbody) {
    if (zqerciboxbody.indexOf("@") == -1) {
        zqboxbodyArr.push(zqerciboxbody)
    } else if (zqerciboxbody.indexOf("@") > -1) {
        zqboxbodys = zqerciboxbody.split("@")
    } else if (process.env.zqerciboxbody && process.env.zqerciboxbody.indexOf('@') > -1) {
        zqboxbodyArr = process.env.zqerciboxbody.split('@');
        console.log(`您选择的是用"@"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqerciboxbody = fs.readFileSync("zqerciboxbody.txt", "utf8");
    if (zqerciboxbody !== `undefined`) {
        zqboxbodys = zqerciboxbody.split("\n");
    } else {
    ...
    if ($request.url.match(/\/kandian.wkandian.com\/v17\/Ad\/getReward/)) {
        ...
        if (zqerciboxbody) {
            if (zqerciboxbody.indexOf(bodyVal) > -1) {
                ...
                $.msg($.name, "获取第" + bodys.length + "个任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqerciboxbody');
            $.log(`${$.name}获取5_60任务: 成功, zqboxbodys: ${bodyVal}`);
            $.msg($.name, `获取第一个5_60任务请求: 成功🎉`, ``)
        }
    }
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success !== false ){
                    console.log('\n领取5-60奖励成功，获得：'+result.items.score + '金币')
                }else{
